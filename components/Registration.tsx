...
function extractCropStage(text: string): UserProfileExtended['present_crop_stage'] {
  const t = text.toLowerCase();
  if (t.includes('seed')) return 'Seedling';
  if (t.includes('veget') || t.includes('growth')) return 'Vegetative';
  if (t.includes('flower')) return 'Flowering';
  if (t.includes('fruit') || t.includes('prod')) return 'Fruiting';
  if (t.includes('harvest') || t.includes('ready')) return 'Harvested';
  return 'Pre-Harvest';
}

const DISEASE_OPTIONS = [
  'Leaf Blight', 'Powdery Mildew', 'Stem Rot', 'Root Rot', 'Rust',
  'Mosaic Virus', 'Aphid Infestation', 'Whitefly', 'Stem Borer', 'None',
];
